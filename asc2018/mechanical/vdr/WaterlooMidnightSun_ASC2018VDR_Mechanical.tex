...
\newcommand\thesubtitle{Mechanical Technical Report} % <--------------- add a subtitle or leave the field empty
...
% 6. Frame/chassis and roll cage type: tubular frame (_x_), composite (___), check one. Drawing shall show the (1) occupants positioned in the frame/chassis, (2) material specs of all metal components, and (3) compliance with Reg 10.3
\section{Chassis}
...
% 7. Roll cage: Profile and frontal drawings shall include material specs and show compliance with Regs 10.3,10.3.B,10.3.C,10.3.G
\section{Roll cage}
...
% 8. Seat Belts: 5 point (_x_), 6 point (___), check one
% Drawing shall indicate location of mounting points and compliance with Reg. 10.3.E
\section{Seat belts}
...
% 9. Braking system: Front wheel only (__x_), Front-rear (___), check one.
% Schematic and description of primary braking system shall include parking brake and component specs demonstrating compliance with Regs.10.5 and 10.6
...
\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/u-joints}
\caption{Torque transmission in steering assembly}
\label{fig:u-joints}
\end{figure}

% 12. Front suspension: type: a-arm (_x_), other (___), check one
% Description shall include drawing/photos, component specs, and engineering analysis demonstrating proper selection and sizing of rod ends with shear loads under applied loads as specified in Appendix F, section F.2
...
The rear suspension pivots about plain bearings pressed into the trailing arm. Stress in the trailing arm at the plain bearings was simulated using FEA as shown Figure \ref{fig:rear-suspension} to verify sufficient safety factor. The lack of rod ends allows a smaller interface with the rear suspension clevis mounts, which would otherwise need to be sized significantly larger. Figure \ref{fig:rear-suspension-cross-section} shows a cross section of the trailing arm clevises.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/rear-suspension-cross-section}
\caption{Cross section of rear suspension}
\label{fig:rear-suspension-cross-section}
\end{figure}

% 14. Tires and rims: Description shall include brand, load, speed, and pressure rating to comply with Regs. 10.2
...
% 15. Hub design: Drawings showing wheel-hub assembly
 \section{Hub Design}
 The hubs for MSXII are designed for the custom rims, and are machined out of 6061-T6 aluminum billet. The hubs are designed to have a slip fit over the rims, and have 6 locating pins which ensure the rim is properly seated and prevent the hub and wheel from rotating independently. The end of the hub body is threaded to accept a matching aluminum hub nut, which is torqued onto the hub and locks the rim and wheel to the hub, and hence to the rest of the suspension assembly. 
 
The hub assembly contains bearings which allow rotation of the wheel and hub about the spindle of the front suspension. The bearings have all been selected to withstand worst case loading on the wheel assembly. Additionally, the hub assembly contains stainless steel brake bobbins, which connect the cast iron brake rotors to the hub assembly and align the rotors with the brake callipers mounted on the suspension assembly. The bobbins provide float to the brake rotors, allowing the rotors to move within a fixed envelope around the hub when the brakes are applied on the car. As the position of the brake callipers is fixed, the float in the rotor allows hard braking to be applied without warping, damaging or shattering the brake rotors. The full hub assembly is shown in figure \ref{fig:front-sus-hub-assy}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/front-sus-hub-assy.png} %<-- UPDATE\
\caption{View of hub assembly}
\label{fig:front-sus-hub-assy}
\end{figure}

% 16. Crush zone: type: foam (__x_); tubular (___),check one
% Description/drawing shall support compliance with Reg. 10.3.F
\section{Crush Zone}
MSXII has been designed with an extruded polystyrene foam (EPS) crush zone, designed to absorb the energy of a 5G impact to the doors on both sides of the vehicle. The EPS crush foam is enclosed between the outer door panel and a load distribution panel positioned outside inner door frame, made of 6061-T6 aluminum tubing, and the inner door panel. EPS foam does not shatter under loading, ensuring no fragments enter the driver cell in the event of a collision. Figures \ref{fig:crush-zone} and  \ref{fig:crush-zone-load-path} show the different layers that make up the doors' structure as well as the key load path for transferring an impact to the structural chassis and roll cage.  

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/crush-zone.png} %<-- UPDATE
\caption{Door assembly showing crush absorbing foam}
\label{fig:crush-zone}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{figures/crush-zone-load-path.png} %<-- UPDATE
\caption{Key structural members in load path}
\label{fig:crush-zone-load-path}
\end{figure}

% 17. Battery box: Description/drawing to show how battery box is constructed and secured
% in the chassis as per Reg. 8.4.B
\section{Battery Box}
Two battery packs are implemented in the design of MSXII: a "slave" pack providing exclusively power and some auxiliary electrical control, and a "master" pack, containing both battery cells and electrical boards which control the battery system. The full details of the electrical side of the pack construction is detailed in the appropriate electrical system VDR. From a mechanical standpoint, both packs have identical designs and mounting features. 

The pack is constructed from 1/4" thick honeycomb fibreglass composite panels, bonded to a welded aluminum frame with an inert epoxy. Fibreglass is electrically insulating, and hence isolates the internals of both battery boxes from the external metal frames and surrounding chassis. The aluminum frame has welded mounting tabs on both the top and bottom of the box, which interface with matching mounting tabs welded to the chassis above and below the box. FEA with a 20G impact load on all mounting tabs yielded safety factors around 1.5-2. This high loading was applied to assure compliance with World Solar Challenge regulations - well above the required 5G impact loading. The mounting tabs provide a location on both sides of each box (master and slave) for a fastener and locking nut to secure the box to the chassis and prevent the battery enclosure from becoming loose during an impact or rollover event. 

%FEA MAYBE GOES HERE?

% 18. Description/drawing to show independent methods of array attachment as per Reg. 10.1.C
\section{Array Panel Attachment}
Both the front and rear array panels are attached to the chassis of MSXII on a custom 6-bar linkage, made from aluminum members and actuated with a gas spring. The upper member of the hinges are attached to the respective array panel with epoxy resin. This serves as the primary method of array attachment to the chassis of the vehicle. As a secondary method to provide redundancy to the first method of array attachment, aluminum aircraft cable is connected to each panel individually, and connected to the chassis. This satisfies the regulation, as each attachment method is independent of one another. 

% 19. Fasteners: Description of compliance with Reg. 10.4
\section{Fasteners}
...
% 20. Vehicle Impact Analysis: Method: Classical (___), FEA (_x_), Testing (___)
% Analysis shall be performed as per Appendix F Section F.3 and the results shall be presented in terms of factor of safety in tabulated form
